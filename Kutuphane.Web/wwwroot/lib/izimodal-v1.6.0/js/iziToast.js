...
(function (root, factory) {
	 ...
	 * @private
	 */
	 $iziToast.progress = function (toast, settings, callback) {
	...
	$iziToast.show = function (options) {
			...
			if(settings.timeout) {
				...
				if(settings.pauseOnHover && !settings.resetOnHover){
					...
					$DOM.toast.addEventListener('mouseenter', function (e) {
						...
						that.progress($DOM.toast, settings, function(){
							that.hide(settings, $DOM.toast, 'timeout');
						}).pause();
		...
		if(settings.title.length > 0 && settings.message.length > 0 && settings.layout !== 2 && !settings.rtl) {
